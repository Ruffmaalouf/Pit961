...
- **Implementation status (P2-WP4 build):** Items (a)-(c) built into Job Detail's new Estimate section (`frontend/features/estimates/EstimateSection.tsx`), following the proposed change as closely as the existing "Work & parts, one list" component's structure allows: a compact line-item editor (add/edit/remove rows, server-recomputed subtotal/total on every save — never client-computed), a single inline discount-percent-plus-Apply control right next to the total (no modal), and a distinct amber (`--status-warning`, the same `#c98a2f` token) "PENDING OWNER APPROVAL" banner with its own Owner-only Clear action, never sharing a button or modal with the discount control. Customer approval is rendered as its own separate card, visually distinct from the Owner-approval banner, matching Owner Decision #2's "these are two different gates" requirement. **Deferred, not yet built:** the Money screen's new Estimates list/filter state (parallel to RECEIVABLE) called for in the proposed change above — P2-WP4's authorized scope was the Job → Estimate workflow; the Money-screen view was not part of the Owner's P2-WP4 order and is left for a future work package rather than built speculatively.
