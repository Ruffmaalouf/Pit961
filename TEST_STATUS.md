...
**Current verified state (2026-08-27):** WP-2, WP-3, WP-3B, and WP-4 application
test suites now exist and pass. **126/126 tests passing** — 5 `GarageOS.Tests.Unit`
(`DemoOptionsBindingTests` from WP-2, `TenantGuardTests` from WP-3,
`GarageInsertBoundaryTests` from WP-3B) + 121 `GarageOS.Tests.Integration`
...
WP-4) — run against real PostgreSQL 15.19 (device-local instance; no Docker/
Testcontainers/PG14/SQLite/InMemory substitute). Migrations
`MakeGaragesAccountActiveIndexUnique` (WP-3B) and `AddUserLockoutColumns` (WP-4)
